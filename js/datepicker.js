...
(function () {
    ...
    function DatePicker() {
        ...
        window.selectDateInit = function (param) {
            config = $.extend({}, defaults, param);
            createHTML();
        ...
        function createHTML() {
            ...
            $year.find("ul").html(createList(config.year.start, config.year.end, ""));
            $month.find("ul").html(createList(config.month.start, config.month.end, ""));
            $day.find("ul").html(createList(config.day.start, config.day.end, ""));

            function createList(min, max, unit) {
                var html = "";
                for (var i = min; i <= max; i++) {
                    html += "<li class='col' data-value=\"" + i + "\">" + i + unit + "</li>";
                }
                return html;
            }
        }
        ...
        function createTouchControls() {
            ...
            function TouchControl($ctrl) {
                ...
                var scrollViewHeight = jQuery($ctrl[0]).height();
                var listHeight = jQuery($list[0]).height();
                var touch = {
                    start: 0,
                    end: 0,
                    current: 0,
                ...
                var limit = {
                    top: itemHeight * 1,
                    bottom: scrollViewHeight - listHeight - itemHeight * 1
                };
                var activeIndex = 0;
            ...
            function getItemHeight() {
                var li = $year.find("ul > li:first-child")
                return jQuery(li).height();
            }

            function getTopByIndex(index) {
                return (2 - index) * itemHeight;
            }

            function getActiveIndex(top, itemHeight) {
                return parseInt(2 - top / itemHeight);
            }
            ...
            function setDate(date) {
                ...
                function createSetDateData(start, current) {
                    var obj = {};
                    obj.value = current;
                    obj.top = (start - current + 1) * itemHeight;
        ...
        function bindActions() {
            ...
            $(document).on(clickEvent, function (e) {
                ...
                //移动端判断是滑动还是点击
                if (isMobile && (touch.x !== e.changedTouches[0].clientX || touch.y !== e.changedTouches[0].clientY || new Date().getTime() - touch.timestamp > 160)) {
                    return;
                }
